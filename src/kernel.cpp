        ...
        MouseToConsole() {
            ...
            VideoMemory[80 * y + x]
                = ((VideoMemory[80 * y + x] & 0xF000)
                   >> 4)  // Move bits at F 4 to right
                | ((VideoMemory[80 * y + x] & 0x0F00)
                   << 4)  // Move bits at F 4 to left
                | ((VideoMemory[80 * y + x]
                    & 0x00FF));  // Draw mouse cursor by flipping character at
                                 // (x, y) to 0xDB
        }
...
extern "C" void kernelMain(void* multiboot_structure, uint32_t magicnumber) {
    ...
    uint32_t* memupper
        = (uint32_t*)(((size_t)multiboot_structure)
                      + 8);  // get memory size from multiboot info structure
    ...
    void* allocated = memoryManager.malloc(100);
    ...
    KeyboardDriver keyboard(
        &interruptManager, &desktop
    );  // attach keyboard to desktop
#else
